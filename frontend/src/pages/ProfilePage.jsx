...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        logger.error("Profile fetch failed:", getErrorMessage(err));
      }
    };
    fetchDetails();
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setDetails(unwrapData(res.data));
      setEditMode(false);
      toast.success("Profile updated successfully");
    } catch (err) {
      logger.error("Profile update failed:", getErrorMessage(err));
  ...
  return (
    <div className="p-6 transition-all duration-300">
      ...
      <div className="col-md-12 col-lg-12 border rounded p-2">
        <InfoCard
          icon={<FiUser size={20} className="text-[var(--primary-color)]" />}
          title="Name"
        >
          {editMode ? (
            <input
        ...
        <InfoCard
          icon={<FiMail size={20} className="text-[var(--primary-color)]" />}
          title="Email"
        >
          {editMode ? (
            <input
        ...
        <InfoCard
          icon={<FiLock size={20} className="text-[var(--primary-color)]" />}
          title="Password"
        ...
        >
          <p className="text-sm text-[var(--muted-text)] mb-2">
            ...
            >
              {details.passwordLastChanged
                ? new Date(details.passwordLastChanged).toLocaleDateString()
                : new Date(details.joinedAt).toLocaleDateString()}
            </span>
          </p>
          <button
        ...
        <InfoCard
          icon={
            <FiCalendar size={20} className="text-[var(--primary-color)]" />
          }
          title="Joined On"
        >
          <p className="text-lg">
